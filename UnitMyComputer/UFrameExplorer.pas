...
uses UMyIcon, UMyUtil, UFrameMyComputer, UMyFaceThread, UFormDestination, shellapi, Clipbrd, UMainForm;
...
begin
  ...
  begin
    if Assigned( vstExplorer.FocusedNode ) and vstExplorer.IsVisible[ vstExplorer.FocusedNode ] then
    begin
...
procedure TFileListRefreshFilterHandle.SelectFirstVisible;
var
  SelectNode : PVirtualNode;
  NodeData : PVstFileListData;
begin
  ...
  if IsShowAllSearch then
    ...
    // 默认选择第一个
  SelectNode := vstFileList.GetFirstVisible;
  if not Assigned( SelectNode ) then
    Exit;
  NodeData := vstFileList.GetNodeData( SelectNode );
  if NodeData.IsLocked then
  begin
    SelectNode := vstFileList.GetNextVisible( SelectNode );
    if not Assigned( SelectNode ) then
      Exit;
  end;
  vstFileList.Selected[ SelectNode ] := True;
